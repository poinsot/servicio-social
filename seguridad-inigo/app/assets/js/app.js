...
(function ($) {
	...
	// Side Nav
	function sideNav() {
		$(document).off('click.sideNavAccordion', '.side-nav .side-nav-menu li a');
		$(document).on('click.sideNavAccordion', '.side-nav .side-nav-menu li a', function(e) {
			if ($(this).parent().children('.dropdown-menu').length === 0) {
				return;
			}
			e.preventDefault();
	...
	function closeSideNavOnClickOutside() {
		$(document).on('click', function (event) {
			if (!$(event.target).closest('.side-nav, .side-nav-toggle').length) {
